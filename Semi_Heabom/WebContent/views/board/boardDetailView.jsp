...
<link rel="stylesheet"
	href="<%= contextPath1 %>/views/board/css/boardDetailView.css"
	type="text/css">
</head>
<body>
	...
	<div class="wrap">
		<div id="main-area">
			<div id="post_header">
				<div id="post_header" style="padding: 0px 30px">
					<div id="notice_category">
						...
						<% } else { %>
						<div>
							<img style="padding-bottom: 4px; width: 20px;"
								src="<%=contextPath%>/resource/img/board/icon/notice.png">
					...
					<div id="user_info">
						<div id="user_profile_img">
							...
							<img style="border-radius: 50%"
								src="<%=contextPath%>/resource/img/profile/기본이미지.png">
							<% } else { %>
							<img style="border-radius: 50%"
								src="<%= contextPath %><%=loginMember.getTitleImg()%>" alt="">
							<% } %>
						</div>
						...
						<div id="user_grade">
							...
							<img style="height: 20px;"
								src="<%=contextPath%>/resource/img/grade/씨앗.gif" alt="">
							<% }else if(loginMember.getGrade().equals("새싹")){ %>
							<img style="height: 20px;"
								src="<%=contextPath%>/resource/img/grade/새싹.gif" alt="">
							<% }else if(loginMember.getGrade().equals("잔디")){ %>
							<img style="height: 20px;"
								src="<%=contextPath%>/resource/img/grade/잔디.gif" alt="">
							<% }else if(loginMember.getGrade().equals("벚꽃")){ %>
							<img style="height: 20px;"
								src="<%=contextPath%>/resource/img/grade/벚꽃.gif" alt="">
							<% }else if(loginMember.getGrade().equals("지구")){ %>
							<img style="height: 20px;"
								src="<%=contextPath%>/resource/img/grade/지구.gif" alt="">
							<% }else if(loginMember.getGrade().equals("태양")){ %>
							<img style="height: 20px; width: auto;"
								src="<%=contextPath%>/resource/img/grade/태양.gif" alt="">
							<% } %>
						</div>
					</div>
					<div id="post_header_footer">
						...
						<div style="text-align: right;" class="post_comment">
							<img style="width: 20px;"
								src="<%= contextPath%>/resource/img/board/icon/reply.png" alt="">
			...
			<div id="post_content">
				...
				<hr
					style="border: 1px solid lightgrey; width: 96%; margin-bottom: 0;">
				<div id="contentImg" style="height: auto">
					<span style="font-size: 18px; font-weight: 800;">첨부파일</span> <img
						style="height: 20px;"
						src="<%= contextPath%>/resource/img/board/icon/attachment.png"
						alt=""> <br> <br>
					<%if (flist.isEmpty()) {%>
					등록된 사진이 없습니다.
					<% } else { %>
				...
				<hr
					style="border: 1px solid lightgrey; width: 96%; margin-bottom: 0;">
				<a style="font-size: 22px; font-weight: 800; padding: 0px 30px;">해시태그
					#</a>
				<div id="post_hashtag" style="height: auto; padding: 0px 30px">
					<%if (hashTag != null) {%>
				...
				<div id="post_content_footer" style="padding: 0px 30px">

					<div id="post_comment2">
						<img src="<%= contextPath%>/resource/img/board/icon/reply.png"
							alt=""> <span id="slideComment">댓글보기 [<%=b.getCountReply()%>]
						</span>
					</div>
					<script>
						$(function(){
					...
					<div id="post_report" data-toggle="modal"
						data-target="#reportModal" style="text-align: right;">
											...
											<div class="post_comment_box">
												<div class="comment_user_info">
													<div class="comment_userid">
														<img style="height:30px; width:30px; border-radius: 50%" src="<%= contextPath%>` + result[i].filePath + result[i].changeName + `" alt="">
														` + result[i].nickname + `
														<img style="height:20px" src="<%= contextPath%>` + result[i].medal + `" alt="">
													</div>
												</div>
												...
												<div class="comment_like">
													<div class="comment_like_img">`
														if (result[i].isLiked == true) {
															html += `<div><img src="<%=contextPath%>/resource/img/board/icon/likeY.png" onclick="likedown(this)"></div>`
														} else {
															html += `<div><img src="<%=contextPath%>/resource/img/board/icon/likeN.png" onclick="likeup(this);"></div>`
														}
					...
					<script>
						function likedown(element) {
							$.ajax({
								url: "likedown.bo",
								data: {
									writer: $(element).parent().next().val()
									, rpno: $(element).parent().next().next().val()
									, loginMem: "<%=loginMember.getMemNo()%>"
								}, 
						
								type: "get", // 요청방식 지정
								success: function (result) { 
									$(element).parent().parent().next().text(result);
									$(element).parent().html(`<img src="<%=contextPath%>/resource/img/board/icon/likeN.png" onclick="likeup(this);">`);
						...
						function likeup(element) {
							$.ajax({
								...
								success: function (result) { //성공시 응답 데이터가 자동으로 매개변수로 넘어온다
									$(element).parent().parent().next().text(result);
									$(element).parent().html(`<img src="<%=contextPath%>/resource/img/board/icon/likeY.png" onclick="likedown(this)">`);
						...
						<div id="write_comment_box" style="width: 100%;">
							...
							<div id="write_comment_footer">
								<input style="text-align: right;"
									class="btn btn-light btn-sm right" type="submit" value="등록">
							</div>
						</div>
						<%
						}
		...
		<div align="right" id="delete-update-area"
			style="width: auto; border: none; margin-top: 10px;">
			<%
			...
			<a href="<%=contextPath%>/updateForm.bo?bno=<%=boardNo%>"
				class="btn btn-sm btn-warning">수정</a> <a onclick="deleteConfirm();"
				...
				<div class="modal-body">
					<form action="<%=contextPath1%>/insert2.rp" method="post">
						...
						<table class="reportForm" border="0">
							<tr>
								<th>신고대상:</th>
								<td><input type="text" id="xUser" name="xUser"
									value="<%=writer%>" readonly
									style="background-color: lightgray;"></td>
							</tr>
							<tr>
								<th>신고글:</th>
								<td><textarea name="xContent" id="xContent" cols="30"
										rows="5" readonly
										style="background-color: lightgray; resize: none;"><%=boardContent%></textarea>
								</td>
							</tr>
							<input type="hidden" name="reNo" value="<%=boardNo%>">
							<input type="hidden" name="reporter"
								value="<%=loginMember.getMemNo()%>">
							<input type="hidden" name="reported"
								value="<%=b.getReportWriter()%>">
							<tr>
								<th>신고종류 :</th>
								<td><input name="reprotTypeSelect" type="hidden"
									onclick="getReportType(event)" value=""> <select
							...
							<tr>
								<th>신고내용 :</th>
								<td><textarea name="reportContent" id="reportContent"
										cols="30" rows="5" style="resize: none;" minlength="1"
										required
										placeholder="신고내용은 신고종류에 맞게 작성해주세요. 부적절한 신고 내용은 신고자가 재제의 대상이 됩니다."></textarea>
								</td>
							</tr>
							<tr>
								<th colspan="2">
									<p style="color: red; margin: 0; text-align: center;">
										무고한 신고는 신고자 계정이 재제 당할 수 있습니다.<input id="userReportCheck"
											type="checkbox" onclick="reportCheck();">
									</p>
								</th>
							</tr>
						</table>
						<br>
						<button type="button" id="thorwReport"
							class="btn btn-sm btn-danger" onclick="insertReport()" disabled>신고하기</button>
